...
当前仓库已经加入 GitHub Container Registry 发布流程，后续推送 `v*` tag 或发布 GitHub Release 时会自动构建并推送：
...
| 参数 | 默认值 | 必填 | 说明 |
| --- | --- | --- | --- |
| `MOVIEMUSE_IMAGE_TAG` | `latest` | 否 | 拉取的镜像标签。正式版本建议固定为 `v1.1.0` 这类版本号。 |
...
从 GitHub Release 下载 Windows 算力端 zip 包。
